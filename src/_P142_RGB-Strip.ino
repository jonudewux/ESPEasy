                                    ...
                                    boolean *ok)
{
  ...
  if (root.success())
  {
    ...
    if (root.containsKey(String(F("eff"))) &&
        root.containsKey(String(F("eff_time"))))
    {
      eff = root.get<String>(String(F("eff")));
      if(root[F("eff")] == String(F("cycle")))
      {
        Plugin_142_cycle = root.get<float>(String(F("eff_time")));
        if (Plugin_142_cycle > 0)
          Plugin_142_cycle = (1.0 / 10.0) / Plugin_142_cycle;   //10Hz based increment value
      }

      if(root[F("eff")] == String(F("none")))
      {
        Plugin_142_cycle = 0;
      }
    } else {
      if (root.containsKey(String(F("state"))))
      {
        if (root[F("state")] == String(F("on")))
        {
          if (root.containsKey(String(F("r"))) &&
              root.containsKey(String(F("g"))) &&
              root.containsKey(String(F("b"))))
            {
              float rgb[3];
              rgb[0] = root.get<float>(String(F("r"))) / 255.0;   //R
              rgb[1] = root.get<float>(String(F("g"))) / 255.0;   //G
              rgb[2] = root.get<float>(String(F("b"))) / 255.0;   //B
      ...
      if (root.containsKey(String(F("brightness"))))
      {
        Plugin_142_hsvDest[2] = root.get<float>(String(F("brightness"))) / 255.0;
        *update = true;
      }

      if (root.containsKey(String(F("transition"))))
      {
        millisFadeTime = root.get<unsigned long>(String(F("transition")));
        *update = true;
      }
    }
    *ok = true;
...
{
  rgb[0] = hsv[2] * mix(1.0, constrain(fabs(fract(hsv[0] + 1.0) * 6.0 - 3.0) - 1.0, 0.0, 1.0), hsv[1]);
  rgb[1] = hsv[2] * mix(1.0, constrain(fabs(fract(hsv[0] + 0.6666666) * 6.0 - 3.0) - 1.0, 0.0, 1.0), hsv[1]);
  rgb[2] = hsv[2] * mix(1.0, constrain(fabs(fract(hsv[0] + 0.3333333) * 6.0 - 3.0) - 1.0, 0.0, 1.0), hsv[1]);
  return rgb;
}
...
float* Plugin_142_rgb2hsv(const float* rgb, float* hsv)
{
  ...
  hsv[0] = fabs(qz + (qw - py) / (6.0 * d + 1e-10));
  hsv[1] = d / (qx + 1e-10);
  hsv[2] = qx;
  return hsv;
}
